[英文原文](<https://medium.com/dailyjs/i-never-understood-javascript-closures-9663703368e8>)
...
你可能会问，是否有任何函数具有闭包，甚至是在全局范围内创建的函数?答案是肯定的。**在全局作用域中创建的函数也具有闭包，但是由于这些函数是在全局作用域中创建的，所以它们可以访问全局作用域中的所有变量，闭包的概念并不重要**。
